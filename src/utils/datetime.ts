...
export function convertDateTimeSyntax(pattern: string): string {
    ...
    const unsupported = (char: string, fallback: string): [string, string] => {
    ...
    const processedParts = parsableParts
        ...
        .replaceAll(...unsupported('L', '')) // 'is leap year'
        .replaceAll(...unsupported('o', 'yyyy')) // ISO year
        ...
        .replaceAll(...unsupported('N', '')) // ISO week day
        .replaceAll(...unsupported('w', '')) // alternate week day
        ...
        .replaceAll(...unsupported('U', '')) // unix timestamp
        .replaceAll(...unsupported('Z', '')) // timezone offset seconds - must be before all other 'Z' replacements
        .replaceAll('e', 'Z') // timezone ID
        .replaceAll('T', 'Z') // timezone abbr
        .replaceAll(...unsupported('I', '')) // 'is DST'
        .replaceAll('o', 'o') // timezone offset like +1200
        .replaceAll('P', 'p') // timezone offset like +12:00
        .replaceAll(...unsupported('t', '')) // days in month
